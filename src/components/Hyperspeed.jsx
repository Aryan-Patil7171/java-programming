  ...
  useEffect(() => {
    ...
    class App {
      ...
      init() {
        ...
        // Listen on the surrounding login card so form controls don't block Hyperspeed interaction.
        this.interactionTarget = this.container.closest('.login-card') || this.container;
        this.interactionTarget.addEventListener('mousedown', this.onMouseDown);
        this.interactionTarget.addEventListener('mouseup', this.onMouseUp);
        this.interactionTarget.addEventListener('mouseleave', this.onMouseUp);

        this.interactionTarget.addEventListener('touchstart', this.onTouchStart, { passive: true });
        this.interactionTarget.addEventListener('touchend', this.onTouchEnd, { passive: true });
        this.interactionTarget.addEventListener('touchcancel', this.onTouchEnd, { passive: true });

        this.interactionTarget.addEventListener('contextmenu', this.onContextMenu);
      ...
      update(delta) {
        ...
        const time = (performance.now() - this.clockStart) / 1000 + this.timeOffset;
